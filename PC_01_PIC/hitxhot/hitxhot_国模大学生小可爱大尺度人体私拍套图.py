...
'https://www.hitxhot.org/gallerys/Z2F4eVdJMy9LVjFoMnduTmF1WGtIdz09.html?page=1'
host = 'https://www.hitxhot.org'

main_url = host + '/gallerys/Z2F4eVdJMy9LVjFoMnduTmF1WGtIdz09.html'

save_pic_util = SavePicUtil.SavePicUtil(session=session)
req_util = ReqUtil.ReqUtil(session=session)

path_output = './OutputData/hitxhot_国模大学生小可爱大尺度人体私拍套图'
...
for page in range(1, 64):
    ...
    if res_get_page:
        ...
        for url in list_pic_url:
            url_pic = url.replace('i0.wp.com/', '').replace('i2.wp.com/', '')
            name_pic = url_pic.split('/')[-1].split('.')[0]
            save_pic_util.save_pic(url=f'{url_pic}', save_dir=path_output, save_name=f'{str(idx).zfill(3)}_{name_pic}', is_async=True)
            idx+=1
