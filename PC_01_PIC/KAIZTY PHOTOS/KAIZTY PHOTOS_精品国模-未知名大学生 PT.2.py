...
main_url = host + '/photos/Q01zUTlwVnhrRkM4cTF3dlIzRHZPdz09.html'

save_pic_util = SavePicUtil.SavePicUtil(session=session)
req_util = ReqUtil.ReqUtil(session=session)

path_output = '../KAIZTY PHOTOS_精品国模-未知名大学生 PT.2'
...
for page in range(1, 20):
    ...
    if res_get_page:
        ...
        for url in list_pic_url:
            url_pic = url.replace('i0.wp.com/', '').replace('i1.wp.com/','')
            name_pic = url_pic.split('/')[-1].split('.')[0]
            name_pic = name_pic[:200] if len(name_pic) > 200 else name_pic
